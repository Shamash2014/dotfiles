...
beautiful.get().wallpaper = string.format("%s/temp/dotfiles/awesome/wallpaper.jpg", os.getenv("HOME"))
...
globalkeys = my_table.join(
    ...
    -- Keyboad switch
    awful.key({ 'Control' }, "space", function () awful.spawn.with_shell("xkb-switch -n") end,
    ...
    awful.key({ modkey }, "x", function ()
        os.execute(string.format("dmenu_run -i -fn 'Monoid' -nb '%s' -nf '%s' -sb '%s' -sf '%s'",
                                 beautiful.bg_normal, beautiful.fg_normal, beautiful.bg_focus, beautiful.fg_focus))
                               end,
      {description = "show dmenu", group = "launcher"})
